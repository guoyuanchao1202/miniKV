...
func TestReElection2A(t *testing.T) {
	...
	// fmt.Printf("%d: =========TestReElection2A: start checkOneLeader=========\n", time.Now().UnixNano())
	leader1 := cfg.checkOneLeader()

	// if the leader disconnects, a new one should be elected.
	// fmt.Printf("%d: =========TestReElection2A: disconnect leader %d and checkOneLeader =========\n", time.Now().UnixNano(), leader1)
	...
	// fmt.Printf("%d: =========TestReElection2A: reconnect old leader %d and checkOneLeader=========\n", time.Now().UnixNano(),leader1)
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()

	// if there's no quorum, no new leader should
	// be elected.
	disconnectServer := (leader2 + 1) % servers
	// fmt.Printf("%d: =========TestReElection2A: disconnect two servers (leader %d, follower %d) and checkNoLeader=========\n", time.Now().UnixNano(), leader2, disconnectServer)
	cfg.disconnect(leader2)
	cfg.disconnect(disconnectServer)
	time.Sleep(2 * RaftElectionTimeout)
	...
	// fmt.Printf("%d: =========TestReElection2A: reconect follower server %d and checkOneLeader=========\n", time.Now().UnixNano(), disconnectServer)
	// if a quorum arises, it should elect a leader.
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()

	// fmt.Printf("%d: =========TestReElection2A: reconect old old leader %d and checkOneLeader=========\n", time.Now().UnixNano(), leader2)
...
func TestManyElections2A(t *testing.T) {
	...
	DPrintf("========Current Leader %d========", leader)
	...
	for ii := 1; ii < iters; ii++ {
		...
		DPrintf("========Disconnect servers (%d, %d, %d)=======", i1, i2, i3)
		...
		cfg.connect(i2)
		cfg.connect(i3)
		DPrintf("========Reconnect servers (%d, %d, %d)======", i1, i2, i3)
	}
...
func TestRejoin2B(t *testing.T) {
	...
	DPrintf("============= Connect OldLeader %d", leader1)
	cfg.connect(leader1)

	cfg.one(104, 2, true)

	// all together now
	DPrintf("============= Connect OldLeader %d", leader2)
	cfg.connect(leader2)
...
func TestBackup2B(t *testing.T) {
	...
	DPrintf("Leader %d, Disconnect server (%d, %d, %d) and submit 50 no-commit messages", leader1, (leader1+2)%servers, (leader1+3)%servers, (leader1+4)%servers)
	...
	log.Printf("Leader %d, Disconnect server (%d, %d)", leader1, (leader1+0)%servers, (leader1+1)%servers)
	cfg.disconnect((leader1 + 0) % servers)
	cfg.disconnect((leader1 + 1) % servers)

	DPrintf("Reconnect server (%d, %d, %d) and submit 50 commit messages", (leader1+2)%servers, (leader1+3)%servers, (leader1+4)%servers)
	...
	DPrintf("Disconnect server %d, commit 50 no-commit messages", other)
	...
	DPrintf("Disconnect all servers and Reconnect server (%d, %d %d) and commit 50 commit messages", (leader1+0)%servers, (leader1+1)%servers, other)
	...
	DPrintf("leader: %d", cfg.checkOneLeader())
...
func TestPersist12C(t *testing.T) {
	...
	DPrintf("============Disconnect and reconnect servers============")
	for i := 0; i < servers; i++ {
		cfg.disconnect(i)
		cfg.connect(i)
	}
	...
	DPrintf("============Disconnect Leader %d and restart and Reconnect", leader1)
	cfg.disconnect(leader1)
	...
	DPrintf("============Disconnect leader %d and commit message and Restart and Reconnect", leader2)
	cfg.disconnect(leader2)
	...
	DPrintf("============Disconnect Server %d and restart and Reconnect", i3)
	cfg.disconnect(i3)
...
func TestFigure8Unreliable2C(t *testing.T) {
	...
	//leaderEnd := cfg.checkOneLeader()
	//DPrintf("Leader: %d", leaderEnd)
